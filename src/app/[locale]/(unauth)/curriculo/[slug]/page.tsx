...
import AgilityLogoRotated from '@/images/icons/agility-logo-rotated.svg';
import BrasilBrand from '@/images/icons/brasil-brand.svg';
...
const CVPage = async ({ params }: { params: Params }) => {
  ...
  return (
    <div className="container">
      ...
      <section>
        <Image src={selectedCV.image} alt="user-cv-image"></Image>
        <Text as="h5" size="2xl">{selectedCV.name}</Text>
        <div className="flex border-separate gap-0 border">
          {selectedCV.jobs.map((job, index) => <Text as="p" size="xs" key={index}>{job.toUpperCase()}</Text>) }
        </div>
        <Text as="p">{selectedCV.personalDescription}</Text>
        <Text as="p">{selectedCV.workDescription}</Text>
      ...
      <section>
        <span className="flex gap-3">
          <Image src={AgilityLogoRotated} alt="agility-logo-rotated"></Image>
          <Text as="h2">
            Habilidades
          </Text>
        </span>
        <div className="flex gap-2 overflow-x-auto scroll-smooth">
          {selectedCV.skills.map((skill, index) => (
            <Card key={index} style="dark" className="flex flex-col justify-start pt-20">
              <Image src={skill.icon} alt="skill-icon"></Image>
      ...
      <section>
        ...
        <div className="flex gap-2 overflow-x-auto scroll-smooth">
          {selectedCV.portfolio.map((project, index) => (
            <Card key={index} style="dark" className="flex flex-col justify-start pt-20">
              <Image src={project.image} alt="project-icon"></Image>
              <Text as="p">{project.name}</Text>
              <Text as="p" size="xs">{project.description}</Text>
            </Card>
          ),
          )}
        </div>
      ...
      <section>
        ...
        <div className="flex gap-2 overflow-x-auto scroll-smooth">
          {selectedCV.recommendations.map((recommendation, index) => (
            <Card key={index} style="dark" className="flex flex-col justify-start pt-20">
              <Image src={recommendation.image} alt="recommendation-icon"></Image>
              <Text as="p">{recommendation.text}</Text>
              <Text as="p" size="xs">{recommendation.author}</Text>
            </Card>
          ),
          )}
        </div>
      ...
      <section>
        <div className="flex flex-col space-y-4 bg-secondaryDarker">
          <Text as="h2" className="leading-relaxed"> Conte-me sobre seu próximo projeto!</Text>
          <Text as="p">
